<#if entries?has_content>
  ...
  <div class="flex-container item-blog" data-id="${entry.getAssetRenderer().getClassPK()}">
          <div class="flex-item-center item-img-container">
              <img class="item-img" alt="" style="width:150px" src="${assetRenderer.getAssetObject().getSmallImageURL(themeDisplay)}"/>
          </div>
          <div class="flex-item-expand flex-item-center item-text">
              <normal class="item-author">${dateUtil.getDate(entry.getPublishDate(), "MMMM dd, yyyy", locale)} - ${entry.getUserName()} </normal> </br>
