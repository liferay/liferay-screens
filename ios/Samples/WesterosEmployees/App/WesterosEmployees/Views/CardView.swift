...
class CardView: UIView {
	...
	func createButton(fontColor: UIColor) -> UIButton {
		...
		let constraintsH = NSLayoutConstraint.constraintsWithVisualFormat("H:|-[button]-|", options: [], metrics: nil, views: views)
		let constraintsY = NSLayoutConstraint.constraintsWithVisualFormat("V:|-[button]-|", options: [], metrics: nil, views: views)

		addConstraints(constraintsH)
		addConstraints(constraintsY)
	...
	override func animationDidStop(theAnimation: CAAnimation, finished flag: Bool) {
		...
		if let backgroundTransform = backgroundTransform {
