...
#import "LRDLFileEntryService_v70.h"

/**
 * @author Bruno Farache
 */
@implementation LRDLFileEntryService_v70
...
- (NSArray *)getFileEntriesWithGroupId:(long long)groupId folderId:(long long)folderId status:(int)status start:(int)start end:(int)end obc:(LRJSONObjectWrapper *)obc error:(NSError **)error {
	...
	[self mangleWrapperWithParams:_params name:@"obc" className:@"com.liferay.portal.kernel.util.OrderByComparator<com.liferay.portlet.documentlibrary.model.DLFileEntry>" wrapper:obc];
...
- (NSDictionary *)updateStatusWithUserId:(long long)userId fileVersionId:(long long)fileVersionId status:(int)status serviceContext:(LRJSONObjectWrapper *)serviceContext workflowContext:(NSDictionary *)workflowContext error:(NSError **)error {
	NSMutableDictionary *_params = [NSMutableDictionary dictionaryWithDictionary:@{
		@"userId": @(userId),
...
- (BOOL)isKeepFileVersionLabelWithFileEntryId:(long long)fileEntryId serviceContext:(LRJSONObjectWrapper *)serviceContext error:(NSError **)error {
	NSMutableDictionary *_params = [NSMutableDictionary dictionaryWithDictionary:@{
		@"fileEntryId": @(fileEntryId),
	}];
